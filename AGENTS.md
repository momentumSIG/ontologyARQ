...
│   ├── CQ_Qwen3.6_objeto_v2/          # CQs generated by Qwen3.6 v2 (30 new, independent)
│   │   └── CQ-object-qwen3.6-v2.md
...
│   │   ├── ontogenia/temp_0_{3,5,7}/
│   │   └── README.md
│   ├── Qwen3.6_objeto_evento/         # Event-centric from Qwen3.6 v2
│   │   ├── memoryless/temp_0_{3,5,7}/
│   │   ├── ontogenia/temp_0_{3,5,7}/
│   │   └── README.md
│   ├── Qwen3.6_objeto_objeto/         # Object-centric from Qwen3.6 v2
│   │   ├── memoryless/temp_0_{3,5,7}/
│   │   ├── ontogenia/temp_0_{3,5,7}/
│   │   └── README.md
...
# Workflow Executed — Qwen3.6 v2 (2026-05-27)

This section documents the ontology generation workflow executed by Qwen3.6 on 2026-05-27, using 30 new competency questions generated independently from all previous CQ sets (Deepseek v2, Kimi-2.6), based on `docs/analisis.docx` gap analysis and `docs/objetoArqueologico/analisis_ontologico_deepseek.md`.

## Step 1 — Competency Question Generation

30 CQs were generated across 6 thematic blocks (5 each). **All 30 CQs are new** — none were reused from `PreguntasCompetencia.docx` or from previous LLM sets. The CQs were designed from scratch with an object-centric core but event-aware modeling, covering gaps identified in `analisis.docx` that were not addressed by previous CQ sets.

| Block | Theme | CQs | Source |
|---|---|---|---|
| 1 | Biography as Historical Entity | 5 | analisis.docx §3.1, Gosden |
| 2 | Analytical Workflows & Scientific Traceability | 5 | analisis.docx §3.2, Guerra, CRMhs |
| 3 | Disputed Interpretation & Multivocality | 5 | analisis.docx §3.3, CRMinf |
| 4 | Taphonomy & Formative Processes | 5 | analisis.docx + GeoSciML integration |
| 5 | Custody, Exhibition & Heritage Patrimonialization | 5 | analisis.docx §4.1, Gill, ICCROM |
| 6 | Object-Context-Territory Correlation | 5 | analisis.docx §4.6, GeoSPARQL |

File: `CQ/CQ_Qwen3.6_objeto_v2/CQ-object-qwen3.6-v2.md`

## Step 2 — Ontology Generation (Event-Centric)
...
```
ontologies_generated/Qwen3.6_objeto_evento/
├── memoryless/
│   ├── temp_0_3/          # 30 .ttl files (one per CQ)
│   ├── temp_0_5/          # 30 .ttl files
│   └── temp_0_7/          # 30 .ttl files
...
| Aspect | Qwen3.6_objeto_evento |
|---|---|
| **Core entities** | Events (BiographicalEvent, AnalyticalWorkflow, TaphonomicAlterationEvent, etc.) |
| **Object role** | Participant in events via `participantObject` |
| **Hierarchy depth** | 1 level (0.3) → 2 levels (0.5) → 3 levels (0.7) |
| **CRMinf integration** | Extensive (I4_Proposition_Set for hypotheses, narratives, claims) |
| **CRMsci integration** | Full (S4_Observation, S19_Encounter, S21_Measurement, S5_Inference_Making) |
| **GeoSciML alignment** | Stratigraphic-geological correlation events |
| **New concepts** | CustodyEvent, HeritageClaimEvent, DigitalDocumentationEvent, FragmentationEvent, GeomorphologicalEvent |

## Step 3 — Ontology Generation (Object-Centric)

The same 30 CQs were used to generate an **object-centric** module with identical directory structure.

### Directory structure

```
ontologies_generated/Qwen3.6_objeto_objeto/
├── memoryless/
│   ├── temp_0_3/          # 30 .ttl files (one per CQ)
│   ├── temp_0_5/          # 30 .ttl files
│   └── temp_0_7/          # 30 .ttl files
...
| Aspect | Qwen3.6_objeto_objeto |
|---|---|
| **Core entities** | Objects (ArchaeologicalObject, NaturalObject, HumanMadeObject) |
| **Event role** | Modeled in relation to object via properties (`hasBiography`, `objectSubjectOfWorkflow`) |
| **Hierarchy depth** | 1 level (0.3) → 2 levels (0.5) → 3 levels (0.7) |
| **Disjointness** | NaturalObject ⊥ HumanMadeObject; AbioticObject ⊥ BioticObject; Artefact ⊥ Structure (0.7) |
| **Reification** | Extensive (NarrativeBiography, InterpretiveHypothesis, HeritageClaim, CertaintyAssessment) |
| **New concepts** | CustodyChain, DigitalReplica, FunctionalZone, OccupationPhase, PaleoenvironmentalCorrelation |

## Step 4 — CRMarchaeo Integration

Both modules extend CRMarchaeo through class subsumption:

**Event-centric class alignment (selected):**
| arqo: class | Superclass | Temperature |
|---|---|---|
| `BiographicalEvent` | `crm:E5_Event` | 0.3+ |
| `ProductionEvent` | `crm:E12_Production` | 0.5+ |
| `DepositionEvent` | `crmarchaeo:A4_Stratigraphic_Genesis` | 0.5+ |
| `RecoveryEvent` | `crmarchaeo:A1_Excavation_Process_Unit` | 0.5+ |
| `TaphonomicAlterationEvent` | `crmarchaeo:A5_Stratigraphic_Modification_Event` | 0.3+ |
| `AnalyticalEncounterEvent` | `crmsci:S19_Encounter` | 0.3+ |
| `InterpretiveHypothesis` | `crminf:I4_Proposition_Set` | 0.3+ |
| `FragmentationEvent` | `crm:E81_Transformation` | 0.3+ |

**Object-centric class alignment (selected):**
| arqo: class | Superclass | Temperature |
|---|---|---|
| `ArchaeologicalObject` | `crm:E19_Physical_Object` | 0.3+ |
| `NaturalObject` | `arqo:ArchaeologicalObject` | 0.5+ |
| `HumanMadeObject` | `arqo:ArchaeologicalObject` | 0.5+ |
| `TaphonomicProcess` | `crmarchaeo:A5_Stratigraphic_Modification_Event` | 0.3+ |
| `PhysicalSample` | `crmsci:S18_Sample` | 0.3+ |
| `DigitalReplica` | `crm:E73_Information_Object` | 0.3+ |
| `FunctionalZone` | `crm:E53_Place` | 0.3+ |

Full inventories in `ontologies_generated/Qwen3.6_objeto_evento/README.md` and `ontologies_generated/Qwen3.6_objeto_objeto/README.md`.
